...
#pragma region Scene Commands
	...
	void Renderer2D::DrawQuadInternal(const Ludus::Math::Transform2D& transform, Color color, Texture* texture, int shape, int fill, bool flipU, bool flipV)
	{
		auto [r, g, b, a] = color;
	...
	void Renderer2D::DrawQuad(const Ludus::Math::Transform2D& transform, Color color, Texture* texture, bool fill)
	{
	...
	void Renderer2D::DrawCircle(const Ludus::Math::Transform2D& transform, Color color, bool fill)
	{
		if (WouldOverflow(4, 6))
		{
			Flush();
	...
	void Renderer2D::DrawText(const Ludus::Math::Transform2D& transform, std::string_view string, Color color, HorizontalTextAlignment horizontalAlignment)
	{
		...
		for (char character : string)
		{
			...
			const Ludus::Math::Vector2D quadCenter { x + width * 0.5f, y + height * 0.5f };
			const Ludus::Math::Vector2D quadScale { width, height };

			DrawQuadInternal(
				Ludus::Math::Transform2D(transform.OwnerHandle, quadCenter, quadScale, transform.Rotation),
				color,
