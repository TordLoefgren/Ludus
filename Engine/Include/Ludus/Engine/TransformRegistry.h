...
namespace Ludus::Engine
{
	...
	using Ludus::Math::TransformHandle;
	using Ludus::Math::Vector2D;

	struct TransformRegistry
	{
	private:
		std::vector<Transform2D> m_Data;										// Object Storage.
		std::vector<TransformHandle> m_Handles;									// Index -> transform handle.
		std::unordered_map<TransformHandle, size_t> m_HandleToIndex;			// Transform handle -> index.
		...
		void RemoveAndReorderIndices(size_t index)
		{
			...
			if (index != lastIndex)
			{
				...
				const TransformHandle movedHandle = m_Handles[index];
	...
	public:
		std::span<const Transform2D> View() const { return { m_Data.data(), m_Data.size() }; }
		std::span<Transform2D> ViewMutable() { return { m_Data.data(), m_Data.size() }; }
		...
		TransformHandle Add(Transform2D transform)
		{
...
#pragma region Lookups
		...
		Transform2D* TryGetByIdMutable(TransformHandle handle)
		{
			if (auto handleIter = m_HandleToIndex.find(handle); handleIter != m_HandleToIndex.end())
			{
				return &m_Data[handleIter->second];
		...
		const Transform2D* TryGetByOwner(GameObjectHandle ownerHandle) const
		{
		...
		Transform2D* TryGetByOwnerMutable(GameObjectHandle ownerHandle)
		{
			if (auto handleIter = m_OwnerHandleToIndex.find(ownerHandle); handleIter != m_OwnerHandleToIndex.end())
			{
				return &m_Data[handleIter->second];
