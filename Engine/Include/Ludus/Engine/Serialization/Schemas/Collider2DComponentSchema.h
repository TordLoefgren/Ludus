...
namespace Ludus::Engine::Serialization::Schemas
{
	...
	struct Collider2DComponentSchema
	{
		...
		inline static Ludus::Engine::Core::Expected<Collider, SerializationException> Deserialize(ITokenStreamReader& reader)
		{
			try
			{
				...
				Ludus::Engine::Serialization::Core::ReadObject(reader,
					[&](std::string_view key)
					{
						if (key == "OwnerHandle")
						{
							collider.OwnerHandle = Ludus::Engine::Serialization::Core::ConsumeAs<Token::Uint32>(reader).Data;
							hasOwner = true;
							return;
						}
						if (key == "LayerIndex")
						{
							collider.LayerIndex = static_cast<uint8_t>(Ludus::Engine::Serialization::Core::ConsumeAs<Token::Uint32>(reader).Data);
							return;
						}
						if (key == "CollidesWith")
						{
							const auto mask = Ludus::Engine::Serialization::Core::ConsumeAs<Token::Uint32>(reader).Data;
			...
			{
				return Ludus::Engine::Core::Expected<Collider, SerializationException>(
