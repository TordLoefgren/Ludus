...
#include <Ludus/Engine/Persistance/Codecs/IDomTextCodec.h>
...
namespace Ludus::Engine::Persistance::Codecs
{
	...
	private:
		...
		static constexpr size_t InlineArrayMax = 8;
		static constexpr size_t InlineObjectMaxPairs = 2;
		static constexpr size_t MaxLineLength = 100;

#pragma region Encode Helpers

		static void AppendIndent(std::string& out, size_t spaces)
		{
			out.append(spaces, ' ');
		}
		...
		static bool CanInlineScalarObject(const DomObject& obj)
		{
			if (obj.empty())
			{
				return true;
		...
		static bool CanInlineDocumentObjectValue(const DomObject& obj)
		{
			return obj.size() == 2 && CanInlineScalarObject(obj);
		}

		static void ValidateNoMixedArrays(const DomArray& arr)
		{
		...
		struct Writer
		{
			...
			{
				// Block list with dashes at current indentation level.
				for (size_t i = 0; i < arr.size(); i++)
				{
		...
		private:
			...
			DomNode ReadDocumentObject(size_t expectedIndent)
			{
				...
				while (LexerInstance.HasMore())
				{
					...
					if (next.Kind == LineKind::ListItem)
					{
						break;
					}
				...
				while (LexerInstance.HasMore())
				{
					...
					if (colon != std::string_view::npos)
					{
						...
						while (LexerInstance.HasMore())
						{
							...
							if (nextPair.Kind == LineKind::Empty)
							{
								break;
							}
							if (nextPair.Indent != expectedIndent + IndentWidth)
							{
								break;
							}
							if (nextPair.Kind == LineKind::ListItem)
							{
								break;
							}
...
#pragma endregion
	...
	public:
		...
		std::string Encode(const DomNode& node) override
		{
			WriterContext context;
		...
		DomNode Decode(const std::string& data) override
		{
			Parser parser(data);
