...
#include <Ludus/Engine/Persistance/Schemas/Collider2DComponentSchema.h>
#include <Ludus/Engine/Persistance/Schemas/DisplayNameComponentSchema.h>
#include <Ludus/Engine/Persistance/Schemas/RigidBody2DComponentSchema.h>
#include <Ludus/Engine/Persistance/Schemas/Sprite2DComponentSchema.h>
#include <Ludus/Engine/Persistance/Schemas/Text2DComponentSchema.h>
...
namespace Ludus::Engine::Persistance::Schemas
{
	...
	private:
		...
		{
			if (!Ludus::Engine::Persistance::Serialization::Is<Token::StartObject>(reader.Peek()))
			{
			...
			if (!result.HasValue())
			{
				if (!Ludus::Engine::Persistance::Serialization::Is<Token::Key>(reader.Peek()) &&
					!Ludus::Engine::Persistance::Serialization::Is<Token::EndObject>(reader.Peek()) &&
					!Ludus::Engine::Persistance::Serialization::Is<Token::EndArray>(reader.Peek()))
				{
					Ludus::Engine::Persistance::Serialization::SkipValue(reader);
				}
	...
	public:
		...
		inline static Ludus::Engine::Core::Expected<Scene, PersistanceException> Deserialize(ITokenStreamReader& reader)
		{
			Scene scene;
			...
			try
			{
				...
				Ludus::Engine::Persistance::Serialization::ReadObject(reader,
					[&](std::string_view key)
					{
						if (key == "Handle")
						{
							scene.Handle = Ludus::Engine::Persistance::Serialization::ConsumeAs<Token::Uint32>(reader).Data;
							hasHandle = true;
							return;
						}
						if (key == "Entities")
							...
							while (!Ludus::Engine::Persistance::Serialization::Is<Token::EndArray>(reader.Peek()))
							{
								...
								Ludus::Engine::Persistance::Serialization::ReadObject(reader,
									[&](std::string_view entityKey)
									{
										if (entityKey == "Handle")
										{
											entityHandle = Ludus::Engine::Persistance::Serialization::ConsumeAs<Token::Uint32>(reader).Data;
				...
				if (!hasHandle)
				{
					throw PersistanceException("No scene handle found.");
				}
			}
