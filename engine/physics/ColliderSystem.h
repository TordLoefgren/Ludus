...
{
private:
	std::vector<Collider2D> m_Data;											// Object Storage.
	std::vector<ColliderHandle> m_Handles;									// Index -> collider handle.
	std::unordered_map<ColliderHandle, size_t> m_HandleToIndex;				// Collider handle -> index.
...
public:
	...
	ColliderHandle Add(GameObjectHandle owner, Index layer, LayerMask collidesWith = LayerMask::GetEmpty(), bool isStatic = false)
	{
	...
	ColliderHandle Add(Collider2D collider)
	{
...
#pragma region Lookups
	...
	Collider2D* TryGetByIdMutable(ColliderHandle handle)
	{
		if (auto handleIter = m_HandleToIndex.find(handle); handleIter != m_HandleToIndex.end())
		{
			return &m_Data[handleIter->second];
	...
	const Collider2D* TryGetByOwner(GameObjectHandle ownerHandle) const
	{
	...
	Collider2D* TryGetByOwnerMutable(GameObjectHandle ownerHandle)
	{
		if (auto handleIter = m_OwnerHandleToIndex.find(ownerHandle); handleIter != m_OwnerHandleToIndex.end())
		{
			return &m_Data[handleIter->second];
