...
namespace Ludus::Graphics
{
	...
	public:
		...
		void DrawQuad(const Transform2D& transform, Color color = Colors::White, Texture* texture = nullptr);
		void DrawLine(float x1, float y1, float x2, float y2, Color color = Colors::White);
		void DrawCircle(const Transform2D& transform, Color color = Colors::White);
		void DrawText(const Transform2D& transform, std::string_view text, Color color = Colors::White);
