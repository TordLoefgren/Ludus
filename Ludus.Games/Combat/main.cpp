...
#pragma region Input Handling
		...
		// Projectile inputs.
		if (window.GetInput().GetKeyDown(Key::Space) && Player1ProjectileFireRate.IsElapsed() && Player1ProjectileCount < MaxActiveProjectiles)
		{
			SpawnProjectile(Player1Handle, Player1ProjectileLayerName, LayerMask::GetMask({ Player2LayerName, BoundaryLayerName }));
			Player1ProjectileFireRate.Reset();
		...
		if (IsMultiplayer)
		{
			if (window.GetInput().GetKeyDown(Key::Enter) && Player2ProjectileFireRate.IsElapsed() && Player2ProjectileCount < MaxActiveProjectiles)
			{
				SpawnProjectile(Player2Handle, Player2ProjectileLayerName, LayerMask::GetMask({ Player1LayerName, BoundaryLayerName }));
				Player2ProjectileFireRate.Reset();
		...
		else
		{
			...
			if (Player2ProjectileFireRate.IsElapsed() && Player2ProjectileCount < MaxActiveProjectiles)
			{
				auto player1Transform = scene.Transforms.TryGetByOwner(Player1Handle);
				auto player2Transform = scene.Transforms.TryGetByOwnerMutable(Player2Handle);
