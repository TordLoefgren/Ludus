...
namespace Pong::States
{
	void ScoreMenuState::Init()
	{
		...
		m_MenuItems.emplace_back("", Ludus::Math::Transform2D(0, { halfWidth - 150.0f, m_GameInfo.WindowOptions.Height - 150.0f }), colorWhite, colorWhite);
		m_MenuItems.emplace_back("New Game", Ludus::Math::Transform2D(0, { halfWidth - 125.0f, halfHeight }));
		m_MenuItems.emplace_back("Exit", Ludus::Math::Transform2D(0, { halfWidth - 50.0f, halfHeight - 100.0f }));
	}
	...
	void ScoreMenuState::Render(float deltaTime)
	{
		...
		m_GameInfo.Renderer.DrawText(header.Transform, scoreText, header.ActiveColor);
